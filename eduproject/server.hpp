...
#ifndef EDUPROJECT_SERVER_HPP
#define EDUPROJECT_SERVER_HPP
...
class server {
 ...
 private:
  void processRequest(asio::ip::tcp::socket &sock) {
    ...
    asio::write(sock, asio::buffer(response_buf, sizeof(response_buf)));

    // Shutting down the request to let the client know that we've sent the whole response.
    sock.shutdown(asio::socket_base::shutdown_send);
  }
};

#endif //EDUPROJECT_SERVER_HPP
